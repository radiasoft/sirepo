...
#
# This is a copy of ../ver-1a/ver-1a.i, modified to run as the sub-application
# of a MOOSE Stochastic Tools Monte Carlo study (see main.i in this directory).
#
# The three physical parameters with the largest experimental uncertainty --
# the diffusivity Arrhenius prefactor and activation energy, and the
# solubility prefactor -- are pulled out to top-level variables so that
# main.i can override them per-sample via MultiAppSamplerControl. Everything
# else is unchanged from ver-1a.i.
...
# Uncertain parameters -- sampled by main.i and overridden on the command
# line via MultiAppSamplerControl. The values below are the nominal
# TMAP4/TMAP7 values and are only used when this file is run standalone.
diffusivity_prefactor = 1.58e-4 # m^2/s
diffusivity_activation_energy = 308000.0 # J/mol
solubility_prefactor = 7.244e22 # at/m^3/Pa (before dividing by temperature)

diffusivity_SiC = '${units ${fparse diffusivity_prefactor*exp(-diffusivity_activation_energy/(R*temperature))} m^2/s -> mum^2/s}'
solubility_constant = '${units ${fparse solubility_prefactor / temperature} at/m^3/Pa -> at/mum^3/Pa}'
...
[Outputs]
  # exodus output is disabled for the stochastic batch runs -- results are
  # pulled directly from the Postprocessors above via SamplerReporterTransfer
  # in main.i, and exodus is expensive to write hundreds of times over.
  # Re-enable (see ../ver-1a/ver-1a.i) if running this file standalone.
  [csv]
    type = CSV
