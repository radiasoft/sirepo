...
SIREPO.app.service('focusPointService', function(plotting, layoutService) {
    ...
    this.invokeDelegatesForFocusPoint = function(delegates, fp, delegateFn, params) {
        ...
        delegates.forEach(function (fpd) {
            if(fpd.focusPoints.indexOf(fp) >= 0) {
                retVals[fpd.name] = fpd[delegateFn].apply(null, params);
            }
        });
        return retVals;
    };
    ...
    this.setupInfoDelegate = function(name, showCompletion, hideCompletion, formatData) {
        ...
        return {
            ...
            showFocusPointInfoComplete: showCompletion || function() {
            },
            ...
            hideFocusPointInfoComplete: hideCompletion ||  function() {
            },
            ...
            formatFocusPointData: formatData || function(focusPoint) {
...
SIREPO.app.directive('interactiveOverlay', function(plotting, focusPointService, keypressService, $window) {
    return {
        ...
        controller: function($scope, $element) {
            ...
            var delegates = $scope.plotInfoDelegates || [];
            var snapshotDelegate = $scope.snapshotDelegate ||
                focusPointService.setupInfoDelegate('snapshotDelegate-' + listenerId);
            delegates.push(snapshotDelegate);
            ...
            function init() {
                ...
                if($scope.focusPoints) {
                    ...
                    snapshotDelegate.focusPoints = $scope.focusPoints;
                }
            ...
            function copyToClipboard() {
                ...
                $scope.focusPoints.forEach(function (fp, fpIndex) {
                    var fpt = snapshotDelegate.formatFocusPointData(fp);
                    if(fpIndex === 0) {
...
SIREPO.app.directive('popupReport', function(plotting, focusPointService, utilities) {
    return {
                ...
                '<g data-is-svg="true" data-ng-drag="true" data-ng-drag-data="focusPoints" data-ng-drag-success="dragDone($data, $event)">',
                    ...
                    '<g class="text-block">',
                        ...
                        // the space value is needed for PNG download on MSIE 11
                            '<g data-ng-attr-id="y-text-{{$index}}">',
                            '</g>',
        ...
        controller: function($scope, $element) {
            ...
            function popupTextSize() {
                return group.select('.text-block').node().getBBox();
            }
            ...
            function popupWindowSize() {
                var tw = popupTextSize().width;
                var th = popupTextSize().height;
                var maxWidth = parseFloat(d3self.attr('width')) - 2 * popupMargin;
                var maxHeight = parseFloat(d3self.attr('height')) - 2 * popupMargin;
                return {
                    width: Math.min(maxWidth, tw + 2 * textMargin),
                    height: Math.min(maxHeight, titleBarHeight + th + 2 * textMargin)
                };
            }

            function refreshText() {
                // format data
                var maxWidth = parseFloat(d3self.attr('width')) - 2 * popupMargin - 2 * textMargin;

                // all focus points share the same x value
                var xText = $scope.plotInfoDelegate.formatFocusPointData($scope.focusPoints[0]).xText;
                var hNode = d3self.select('.hidden-txt-layout');
            ...
            function showPopup(geometry, isReposition) {
                ...
                var borderWidth = utilities.fontSizeFromString(rptWindow.style('stroke-width'));
    ...
    return {
        ...
        controller: function($scope) {
            ...
            $scope.popupDelegate = focusPointService.setupInfoDelegate(
                ...
                formatFocusPointData
            );
            $scope.snapshotDelegate = focusPointService.setupInfoDelegate(
                $scope.modelName + '-snapshot-delegate',
                null,
                null,
                formatFocusPointData
            );
            ...
            $scope.focusCircleDelegate = focusPointService.setupInfoDelegate(
                $scope.modelName + '-circle-delegate',
                null,
                null,
                null
            );
...
SIREPO.app.directive('plot3d', function(appState, plotting, utilities, focusPointService, layoutService, keypressService) {
    return {
        ...
        controller: function($scope) {
            ...
            $scope.focusCircleDelegateBottom = focusPointService.setupInfoDelegate(
                $scope.modelName + '-circle-delegate-bottom',
                showFocusPointText,
                hideFocusPointText,
                null
            );

            $scope.focusCircleDelegateRight = focusPointService.setupInfoDelegate(
                $scope.modelName + '-circle-delegate-right',
                showFocusPointText,
                hideFocusPointText,
                null
            );
            ...
            function showFocusPointText(focusPoint) {
                ...
                var focusText = select('.focus-text');
                var fmtTxt = formatFocusPointData(focusPoint);
            ...
            $scope.load = function(json) {
                ...
                plots.forEach(function (plot, i) {
                    ...
                    if(! $scope.focusPoints[i]) {
                        ...
                        var fcd = focusPointService.setupInfoDelegate(
                            $scope.modelName + '-circle-delegate-' + i,
                            null,
                            null,
                            null
                        );
    ...
    return {
        ...
        controller: function($scope, $element) {
            ...
            function refresh(doCacheCanvas) {
                ...
                /*
                var xsort = cornerSorter(0);
                if(! vtkPlotting.updateAxis(
                    boundAxes.x,
                    cornerSelector(
                        [vpOutline.getExtrema().lowestCorners, vpOutline.getExtrema().highestCorners],
                        vtkCanvasHolderBounds
                    ...
                    vtkPlotting.updateAxis(boundAxes.x, centerSelector(vtkCanvasHolderBounds), xsort);
                }
                */
                //
                //testSorter(csort, [[2,1], [1,2]]);
                //var csel = cornerSelector(
                //    [vpOutline.getExtrema().lowestCorners, vpOutline.getExtrema().highestCorners],
                //    vtkCanvasHolderBounds
                //);
                //var eArr = boundAxes.x.getEdges();
                //testSelector(csel, eArr, csort);
                ...
                //geometry.basis.forEach(function (dim) {
                    ...
                    srdbg(dim, 'ext', externalEdges);
                    var isReversed = vpOutline.isReversed()[dim];
                    var orientation = axisCfg[dim].orientation;

                    // 1: increasing coordinate -> increasing value
                    // -1: increasing coordinate -> decreasing value
                    var sense = orientation === vtkPlotting.orientations.horizontal ? 1 : -1;
                    ...
                    if(! seg) {
                        ...
                        seg = geometry.bestLineSegmentForEdges([cl], screenRect, dim, rev);
                    }
                    if(! seg) {
                        // don't draw axes
                    ...
                    var newRange = Math.min(seg.full.length(), seg.clipped.length());
                    var angle = (180 * Math.atan(seg.clipped.slope()) / Math.PI);
                    // this orientation starts out vertical
                    if(orientation === 'left' || orientation === 'right') {
                        angle -= 90;
                        if (angle < -90) {
                            angle += 180;
                    ...
                    var allPts = seg.full.points().concat(seg.clipped.points());
                    var aps = geometry.sortInDimension(allPts, dim, rev);

                    // 1st 2, last 2 points
                    var zs = isReversed ? [axisCfg[dim].max, axisCfg[dim].min] : [axisCfg[dim].min, axisCfg[dim].max];
                    ...
                    for(var i = 0; i < aps.length; i += 2) {
                        ...
                        if(d != 0) {
                            ...
                            var k = sense * (isReversed ? (1 + sense) / 2 - j : j - (1 - sense) / 2);
                            var z = zs[j];
                            var otherz = zs[1 - j];
                            var dp = d / seg.full.length();
                            var part = (z - otherz) * dp;
                            var newz = sense * (z - part);
                            newDom[k] = newz;
                        }
                    }
                    //srdbg(dim, 'dom', [newMin, newMax], 'newDom', newdom);
                    ////axes[dim].scale.domain(newDom).nice();
                    ////axes[dim].scale.range([isReversed ? newRange : 0, isReversed ? 0 : newRange]);

                    ////if (showAxisEnds) {
                    ////    axes[dim].svgAxis.ticks(0);
                    ////    select('.' + dim + '.axis').call(axes[dim].svgAxis);
                    ////}
                    ////else {
                    ...
                    var xform = 'translate(' +
                        ...
                        'rotate(' + angle + ')';
                    //srdbg('xxf', xform);
                    ////select('.x.axis').attr('transform', xform);

                    var dimLabel = axisCfg[dim].dimLabel;
                    /*
                    d3self.selectAll('.' + dim + '.axis-end')
                        .style('opacity', showAxisEnds ? 1 : 0);

                    select(axisSelector + '-end.low')
                        .text('◄ ' + dimLabel + ' ' + axes[dim].svgAxis.tickFormat()(isReversed ? newMax : newMin) + axes[dim].unitSymbol + axes[dim].units)
                        .attr('x', xAxisLeft)
                        .attr('y', xAxisTop)
                        .attr('transform', 'rotate(' + (angle) + ', ' + xAxisLeft + ', ' + xAxisTop + ')');

                    select(axisSelector + '-end.high')
                        .attr('text-anchor', 'end')
                        .text(axes[dim].svgAxis.tickFormat()(isReversed ? newDom[1] : newDom[0]) + axes[dim].unitSymbol + axes[dim].units + ' ' + dimLabel + ' ►')
                        .attr('x', xAxisRight)
                        .attr('y', xAxisBottom)
                        .attr('transform', 'rotate(' + (angle) + ', ' + xAxisRight + ', ' + xAxisBottom + ')');

                    // counter-rotate the tick labels
                    var xlabels = d3self.selectAll('.' + dim + '.axis text');
                    xlabels.attr('transform', 'rotate(' + (-angle) + ')');
                    select('.x.axis .domain').style({'stroke': 'none'});
                    //select('.x.axis').style('opacity', newRange < minAxisDisplayLen ? 0.0 : 1.0);
                    ...
                    select('.x-axis-label')
                        ...
                        //.style('opacity', showXAxisEnds || xrange < minAxisDisplayLen ? 0.0 : 1.0);
                        */
                };
                // vvvvvv
                var dim = 'x';
                var rev = false;
                var sense = 1;  // increasing coordinate -> increasing value
                var cl = null;
                var angle = 0;
                ...
                seg = geometry.bestLineSegment(boundEdges[dim], [lwc, hgc], screenRect, dim, rev);
                var edges = edgeProps.edges;
                var tanPsi = 0;
                isXReversed = vpLeftRight[0][0] > vpLeftRight[1][0];
                if (edges) {
                    sceneXEnds = edgeProps.sceneEnds;
                    sceneXLen = edgeProps.sceneLen;
                    clippedXEnds = edgeProps.clippedEnds;
                    // left end of the x axis is the rightmost left end from the scene vs clipped ends
                    ...
                    showXAxisEnds = true;
                    sceneXEnds = plotUtilities.sortInDimension(vpLeftRight, 0);
                    screenXEnds = plotUtilities.boundsIntersections(vtkCanvasHolderBounds, sceneXEnds[0], sceneXEnds[1]);
                    clippedXEnds = plotUtilities.sortInDimension(
                        plotUtilities.edgesInsideBounds(screenXEnds, vtkCanvasHolderBounds),
                        0, false);
                    if(clippedXEnds) {
                        xAxisLeft = Math.max(sceneXEnds[0][0], clippedXEnds[0][0]);
                        xAxisTop = xAxisLeft == sceneXEnds[0][0] ? sceneXEnds[0][1] : clippedXEnds[0][1];
                        ...
                        xAxisLeft = sceneXEnds[0][0];
                        xAxisTop = sceneXEnds[0][1];
                        xAxisRight = sceneXEnds[1][0];
                        xAxisBottom = sceneXEnds[1][1];
                    }
                    xAxisProjLen = plotUtilities.dist([xAxisLeft, xAxisTop], [xAxisRight, xAxisBottom]);
                ...
                if(! seg) {
                    ...
                    //srdbg('x offscreen');
                    cl = vpOutline.vpCenterLineForDimension(dim);
                    seg = geometry.bestLineSegment([cl], [], screenRect, dim, rev);
                    //srdbg('offscreen x seg', seg.full.str(), seg.clipped.str());
                }
                var isr = vpOutline.isReversed();
                //srdbg('isr', isr);
                isReversed = isr[dim];
                orientation = axes[dim].svgAxis.orient();
                angle = (180 * Math.atan(seg.clipped.slope()) / Math.PI);
                // this orientation starts out vertical
                if(orientation == 'left' || orientation == 'right') {
                    angle -= 90;
                    if (angle < -90) {
                ...
                //srdbg('x seg full', seg.full.str(), 'clipped', seg.clipped.str(), 'angle', angle);
                //srdbg(dim, 'seg full', seg.full.str(), 'clipped', seg.clipped.str(), 'angle', angle);
                //srdbg(dim, 'angle', angle, 'rev', isReversed, 'xangle', xAxisAngle, 'xrev', isXReversed);
                //srdbg(dim, 'rev', isReversed, 'xrev', isXReversed);
                //if( (isReversed && ! isXReversed) || (! isReversed && isXReversed) ) {
                //    srdbg(dim, 'REV DISAGREEMENT: ', isReversed, '!=', isXReversed);
                //}
                ...
                if (!plotUtilities.isPointWithinBounds(sceneXEnds[0], vtkCanvasHolderBounds)) {
                    domainPct = clippedXEnds ? plotUtilities.dist(sceneXEnds[0], clippedXEnds[0]) / sceneXLen : 0;
                ...
                if (!plotUtilities.isPointWithinBounds(sceneXEnds[1], vtkCanvasHolderBounds)) {
                    domainPct = clippedXEnds ? plotUtilities.dist(sceneXEnds[1], clippedXEnds[1]) / sceneXLen : 0;
                    domainPart = (zmax - zmin) * domainPct;
                    if (isXReversed) {
                        newMin = zmin + domainPart;
                    }
                ...
                if (showXAxisEnds) {
                    axes.x.svgAxis.ticks(0);
                    select('.x.axis').call(axes.x.svgAxis);
                }
                else {
                    axes.x.updateLabelAndTicks({
                        width: xrange,
                ...
                //TODO(mvk): adjust axis position to account for tick labels
                ...
                select('.x.axis-end.low')
                    .text('◄ z ' + axes.x.svgAxis.tickFormat()(isXReversed ? newMax : newMin) + axes.x.unitSymbol + axes.x.units)
                    .attr('x', xAxisLeft)
                    .attr('y', xAxisTop)
                    .attr('transform', 'rotate(' + (xAxisAngle) + ', ' + xAxisLeft + ', ' + xAxisTop + ')');
                select('.x.axis-end.high')
                    .attr('text-anchor', 'end')
                    .text(axes.x.svgAxis.tickFormat()(isXReversed ? newMin : newMax) + axes.x.unitSymbol + axes.x.units + ' z ►')
                    .attr('x', xAxisRight)
                    .attr('y', xAxisBottom)
                    .attr('transform', 'rotate(' + (xAxisAngle) + ', ' + xAxisRight + ', ' + xAxisBottom + ')');
                ...
                //boundAxes.y.update(
                //    vtkCanvasHolderBounds,
                //    [vpOutline.getExtrema().leftmostCorners, vpOutline.getExtrema().rightmostCorners]
                //);
                /*
                var ysort = cornerSorter(1);
                if(! vtkPlotting.updateAxis(
                    boundAxes.y,
                     cornerSelector(
                        [vpOutline.getExtrema().leftmostCorners, vpOutline.getExtrema().rightmostCorners],
                    ...
                    vtkPlotting.updateAxis(boundAxes.y, centerSelector(vtkCanvasHolderBounds), ysort);
                }
                */
                ...
                if (edges) {
                    ...
                    showYAxisEnds = true;
                    //isYReversed = vpBottomTop[0][1] < vpBottomTop[1][1];
                    sceneYEnds = plotUtilities.sortInDimension(vpBottomTop, 1);
                    screenYEnds = plotUtilities.boundsIntersections(vtkCanvasHolderBounds, sceneYEnds[0], sceneYEnds[1]);
                    clippedYEnds = plotUtilities.sortInDimension(
                        plotUtilities.edgesInsideBounds(screenYEnds, vtkCanvasHolderBounds),
                        1, false);
                    if(clippedYEnds) {
                        yAxisTop = Math.max(sceneYEnds[0][1], clippedYEnds[0][1]);
                        yAxisLeft = yAxisTop == sceneYEnds[0][1] ? sceneYEnds[0][0] : clippedYEnds[0][0];
                        ...
                        yAxisTop = sceneYEnds[0][1];
                        yAxisLeft = sceneYEnds[0][0];
                        yAxisBottom = sceneYEnds[1][1];
                        yAxisRight = sceneYEnds[1][0];
                    }
                    yAxisProjLen = plotUtilities.dist([yAxisLeft, yAxisTop], [yAxisRight, yAxisBottom]);
                ...
                if(! seg) {
                    // all desired edges are offscreen, so
                    //srdbg('y offscreen');
                    var clsy = vpOutline.vpCenterLineForDimension(dim);
                    seg = geometry.bestLineSegment([clsy], [], screenRect, dim, rev);
                    //srdbg('offscreen x seg', seg.full.str(), seg.clipped.str());
                }
                //srdbg('y seg', seg,  'full', seg.full.str(), 'clipped', seg.clipped.str());
                orientation = axes[dim].svgAxis.orient();
                angle = (180 * Math.atan(seg.clipped.slope()) / Math.PI);
                if(orientation == 'left' || orientation == 'right') {
                    angle -= 90;
                ...
                if (!plotUtilities.isPointWithinBounds(sceneYEnds[0], vtkCanvasHolderBounds)) {
                    domainPct = clippedYEnds ? plotUtilities.dist(sceneYEnds[0], clippedYEnds[0]) / sceneYLen : 0;
                ...
                if (!plotUtilities.isPointWithinBounds(sceneYEnds[1], vtkCanvasHolderBounds)) {
                    domainPct = clippedYEnds ? plotUtilities.dist(sceneYEnds[1], clippedYEnds[1]) / sceneYLen : 0;
                    domainPart = (xmax - xmin) * domainPct;
                    if (isYReversed) {
                        newMax = xmax - domainPart;
                    }
                ...
                for(var i = 0; i < aps.length; i += 2) {
                    ...
                    if(d !== 0) {
                        var j = Math.floor(i / 2);
                        //var k = isYReversed ? j : 1 - j;
                        var k = sense * (isYReversed ? (1 + sense) / 2 - j : j - (1 - sense) / 2);
                        var x = xs[j];
                        var otherx = xs[1 - j];
                        var dp = d / seg.full.length();
                        var part = (x - otherx) * dp;
                        var newx = sense * (x - part);
                ...
                */
                ...
                d3self.selectAll('.y.axis-end')
                    .style('opacity', showYAxisEnds ? 1 : 0);
                select('.y.axis-end.low')
                //.attr('text-anchor', 'end')
                    .text('▼ x ' + axes.y.svgAxis.tickFormat()(isYReversed ? newMax : newMin) + axes.y.unitSymbol + axes.y.units)
                    .attr('x', yAxisRight)
                    .attr('y', yAxisBottom)
                    .attr('transform', 'rotate(' + (yAxisAngle) + ', ' + yAxisRight + ', ' + yAxisBottom + ')');
                select('.y.axis-end.high')
                    .text('▲ x ' + axes.y.svgAxis.tickFormat()(isYReversed ? newMin : newMax) + axes.y.unitSymbol + axes.y.units)
                    .attr('x', yAxisLeft)
                    .attr('y', yAxisTop)
                    .attr('transform', 'rotate(' + (yAxisAngle) + ', ' + yAxisLeft + ', ' + yAxisTop + ')');
                ...
                select('.y-axis-label')
                    ...
                    .style('opacity', showYAxisEnds || yrange < minAxisDisplayLen ? 0.0 : 1.0);
                ...
                //srdbg('getting z seg');
                seg = geometry.bestLineSegment(boundEdges[dim], [zl, zr], screenRect, dim, rev);
                //srdbg('got z seg');
                edges = edgeProps.edges;
                var tanTheta = 0;
                ...
                if(! seg) {
                    // all desired edges are offscreen, so
                    var clsz = vpOutline.vpCenterLineForDimension(dim);
                    seg = geometry.bestLineSegment([clsz], [], screenRect, dim, rev);
                    //srdbg('offscreen x seg', seg.full.str(), seg.clipped.str());
                }
                //isZReversed = seg.clipped.points()[0];
                orientation = axes[dim].svgAxis.orient();
                angle = (180 * Math.atan(seg.clipped.slope()) / Math.PI);
                if(orientation == 'left' || orientation == 'right') {
                    angle -= 90;
                ...
                //srdbg('z seg', seg,  'full', seg.full.str(), 'clipped', seg.clipped.str(), 'angle', angle);

                var zAxisLineSeg = geometry.lineSegment(
                    geometry.point(zAxisLeft, zAxisTop),
                    geometry.point(zAxisRight, zAxisBottom)
                );
                //srdbg('z axis seg', zAxisLineSeg.str(), 'axis angle', zAxisAngle);

                function pickdim(arr, dim) {
                    return arr.map(function (d) {
                        return d[dim];
                    });
                }
                var xbs = [];  var ybs = [];
                [sceneXEnds, sceneYEnds, sceneZEnds].forEach(function (ends) {
                    xbs = xbs.concat(pickdim(ends, 0));
                    ybs = ybs.concat(pickdim(ends, 1));
                });

                // bounding rectangle of the scene
                sceneRect = geometry.rect(
                    geometry.point(Math.min.apply(null, xbs), Math.min.apply(null, ybs)),
                    geometry.point(Math.max.apply(null, xbs), Math.max.apply(null, ybs))
                );
                ...
                var zrn = Math.min(seg.full.length(), seg.clipped.length());
                //srdbg('zrange', zrange, 'seg zr', zrn);
                ...
                // 1st 2, last 2 points
                // map z values to segment?  in axis code?
                var ys = isZReversed ? [ymax, ymin] : [ymin, ymax];
                newdom = [ymin, ymax];
                for(var i = 0; i < aps.length; i += 2) {
                    var p1 = aps[i];
                    var p2 = aps[i+1];
                    ...
                    if(d !== 0) {
                        var j = Math.floor(i / 2);
                        var k = sense * (isZReversed ? (1 + sense) / 2 - j : j - (1 - sense) / 2);
                        var y = ys[j];
                        var othery = ys[1 - j];
                        var dp = d / seg.full.length();
                        var part = (y - othery) * dp;
                        var newy = sense * (y - part);
                        //srdbg(dim, 'recalc dom', i, p1, p2, d, dp, 'old', x, 'other', otherx, 'part', part, 'new', newx);
                        newdom[k] = newy;
                    }
                }
                //srdbg('zdom', [newMin, newMax], 'newdom', newdom, 'rev?', isZReversed);
                ...
                //srdbg('cam angles', cam.getViewUp(), cam.getPosition());
                if(doCacheCanvas) {
                    cacheCanvas();
                }
            }
            ...
            function cornerSelector(cornerArr, bounds) {
                var selector = {};
                var selection = {};
