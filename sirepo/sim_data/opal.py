...
class SimData(sirepo.sim_data.SimDataBase):
    ...
    def _remove_deprecated_items(cls, models):
        cmds = []
        deprecated_cmds = set([
            'attlist', 'eigen', 'envelope', 'list', 'matrix',
            'micado', 'start', 'survey', 'threadall', 'threadbpm',
            'twiss', 'twiss3', 'twisstrack',
        ])
        ...
        deprecated_elements = set([
            'CYCLOTRONVALLEY',
            'HMONITOR', 'INSTRUMENT', 'MULTIPOLETCURVEDCONSTRADIUS', 'MULTIPOLETCURVEDVARRADIUS',
            'MULTIPOLETSTRAIGHT', 'PARALLELPLATE', 'PATCH', 'PEPPERPOT', 'SLIT', 'SROT',
            'SEPARATOR', 'STRIPPER', 'VMONITOR', 'WIRE', 'YROT',
        ])
        removed_ids = []
