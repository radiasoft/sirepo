...
_VTI_TEMPLATE = PKDict(
    cellData=PKDict(arrays=[], vtkClass="vtkDataSetAttributes"),
    fieldData=PKDict(arrays=[], vtkClass="vtkDataSetAttributes"),
...
def _extract_dcm_info(files, info, frame):
    if info is None:
        ...
        if "PatientPosition" in frame:
            ...
            # rtdose
            info.SliceThickness = float(frame.GridFrameOffsetVector[1]) - float(
                frame.GridFrameOffsetVector[0]
            )
            info.Count = int(frame.NumberOfFrames)
    elif "SliceThickness" not in info:
        info.SliceThickness = abs(
            info.ImagePositionPatient[2] - frame.ImagePositionPatient[2]
        )
    return info
...
def _write_ct_vti_file(files, prefix):
    ...
    if is_flipped_lr:
        origin[0] = first.ImagePositionPatient[0] - first.PixelSpacing[0] * (
            first.Columns - 1
        )
