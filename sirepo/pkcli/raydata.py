...
def replay(source_catalog, destination_catalog, num_scans):
    def _emit_new_document(
        ...
        if document_name == "event_page":
            for e in event_model.unpack_event_page(original_document):
                _emit_new_document(
                    destination_catalog_name, new_uid, "event", e, broker
                )
        elif document_name == "datum_page":
            for d in event_model.unpack_datum_page(original_document):
                _emit_new_document(
                    destination_catalog_name, new_uid, "datum", d, broker
                )
        else:
            f = PKDict(uid=new_uid)
    ...
    def _get_new_document(document_name, original_document, updated_fields):
        ...
        if document_name == "start":
            n["uid"] = updated_fields["uid"]
        elif "run_start" in n:
            n["run_start"] = updated_fields["uid"]
        return n

    def _replay_scan(new_uid, old_uid, source_catalog_name, destination_catalog_name):
        ...
        for n, d in databroker.catalog[source_catalog_name][old_uid].documents(
            fill="no"
        ):
            _emit_new_document(destination_catalog_name, new_uid, n, d, b)
...
def scan_monitor():
    ...
    class _Analysis(_DbBase):
        ...
        def analysis_output_dir(cls, uid):
            return sirepo.feature_config.cfg().raydata.db_dir.join(uid)
        ...
        def scans_with_status(cls, catalog_name, statuses):
            ...
            for s in statuses:
                for x in cls.search_all_by(catalog_name=catalog_name, status=s):
                    r.append(
                        PKDict(uid=x.uid, status=x.status, catalog_name=catalog_name)
                    )
            return r
        ...
        def _analysis_output(self, req):
            ...
            def _paths(uid):
                return pkio.sorted_glob(
                    _Analysis.analysis_output_dir(uid).join("**/*.png")
                )
        ...
        def _get_scans(self, req):
            ...
            if a.analysisStatus == "allStatuses":
                ...
                l = _Analysis.scans_with_status(
                    a.catalogName, _AnalysisStatus.NON_STOPPED
                )
    ...
    async def _init_analysis_processors():
        ...
        ] * sirepo.feature_config.cfg().raydata.concurrent_analyses
    ...
    async def _init_catalog_monitors():
        ...
        for c in sirepo.feature_config.cfg().raydata.catalog_names:
    ...
    def _notebook_zip_path(catalog_name):
        return sirepo.feature_config.cfg().raydata.notebook_dir.join(
            f"{catalog_name}.zip"
        )
    ...
    pkio.mkdir_parent(sirepo.feature_config.cfg().raydata.db_dir)
    _Analysis.init(sirepo.feature_config.cfg().raydata.db_dir.join("analysis.db"))
    l = pkasyncio.Loop()
