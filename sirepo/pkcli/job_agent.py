...
#TODO(robnagler) commands need their own init hook like the server has
    ...
    cfg = pkconfig.init(
        ...
        supervisor_sim_db_file_key=pkconfig.Required(
            str,
            'key for supevisor simulation db file access',
        ),
    ...
    def job_cmd_env(self, env=None):
        return job.agent_env(
            env=(env or PKDict()).pksetdefault(
                ...
                SIREPO_SIM_DATA_SUPERVISOR_SIM_DB_FILE_KEY=cfg.supervisor_sim_db_file_key,
            ),
        )
