...
def do_form(form, sreq):
    ...
    data = read_zip(base64.decodebytes(pkcompat.to_bytes(form['zip'])), sreq)
    data.models.simulation.folder = '/Import'
    data.models.simulation.isExample = False
    return simulation_db.save_new_simulation(data)


def read_json(text, sreq, sim_type=None):
    ...
    return sreq.parse_post(req_data=data).req_data


def read_zip(zip_bytes, sreq, sim_type=None):
        ...
        with zipfile.ZipFile(six.BytesIO(zip_bytes), 'r') as z:
            for i in z.infolist():
                ...
                if b.lower() == simulation_db.SIMULATION_DATA_FILE:
                    ...
                    data = read_json(c, sreq, sim_type)
                    continue
