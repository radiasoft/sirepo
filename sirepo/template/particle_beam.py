...
def analyze_ptc_beam(ptc_beam_data, mc2 = m_p_GeV):
    ...
    for i in range(0, len(ptc_beam_data)):
        ...
        ptc_beam_summary.append([s0, n,
            xx, xpx, xy, xpy, xct, xpt, pxpx, pxy, pxpy, pxct, pxpt,
            yy, ypy, yct, ypt, pypy, pyct, pypt, ctct, ctpt, ptpt,
            W, x0, px0, y0, py0, ct0, pt0,
            eta_x, eta_px, eta_y, eta_py])

    ptc_beam_array = np.asarray(ptc_beam_summary)

    ptc_beam_dict = {}
    keys = ['s', 'n', 'xx', 'xpx', 'xy', 'xpy', 'xct', 'xpt',
            'pxpx', 'pxy', 'pxpy', 'pxct', 'pxpt',
            'yy', 'ypy', 'yct', 'ypt', 'pypy', 'pyct', 'pypt', 'ctct', 'ctpt', 'ptpt',
            'W', 'x0', 'px0', 'y0', 'py0', 'ct0', 'pt0',
            'eta_x', 'eta_px', 'eta_y', 'eta_py']
    ...
    ptc_beam_dict['emit_x']  = np.sqrt(ptc_beam_dict['xx']   * ptc_beam_dict['pxpx'] - ptc_beam_dict['xpx']  ** 2.)
    ptc_beam_dict['emit_y']  = np.sqrt(ptc_beam_dict['yy']   * ptc_beam_dict['pypy'] - ptc_beam_dict['ypy']  ** 2.)
    ptc_beam_dict['emit_t']  = np.sqrt(ptc_beam_dict['ctct'] * ptc_beam_dict['ptpt'] - ptc_beam_dict['ctpt'] ** 2.)
...
def populate_uncoupled_beam(num_particles,
                            beta_x, alpha_x, emittance_x,
                            beta_y, alpha_y, emittance_y,
                            dtau, dpt, iseed = None):
    """
    ...
    mean = np.zeros(4)
    cov = np.identity(4)
    for i in [0, 1]: cov[i,i] = ex
    for i in [2, 3]: cov[i,i] = ey
    transverse = np.matmul(normalizing_A,
                           rng.multivariate_normal(mean, cov, n_ptcl).T)
    x  = transverse[0]
    xp = transverse[1]
    y  = transverse[2]
    yp = transverse[3]
