...
_DIM_PLOT_COLORS = ["#d0c383", "#9400d3"]
    ...
    for f in ("y1", "y2", "y3"):
        ...
        plots.append(
            PKDict(
                ...
                style="line"
                if "action" in report and report.action == "fft"
                else "scatter",
            )
        )
...
def get_fft(run_dir, data):
    ...
    return template_common.parameter_plot(
        ...
        PKDict(
            ...
            summaryData=PKDict(
                freqs=found_freqs, minFreq=w_list[0], maxFreq=w_list[-1]
            ),
...
def export_jupyter_notebook(data):
    ...
    while rpt_name in data.models:
        ...
        while y_var in rpt:
            ...
            y_info.append(
                PKDict(y_var=y_var, y_index=y_index, y_label=y_label, style="line")
            )
            code.append(
                (
                    f"{y_var} = "
        ...
        if j == 0:
            ...
            nb.add_report(
                PKDict(
                    x_var=x_var, x_label=x_label, y_info=y_info, title="Analysis Plot"
                )
            )
        ...
        if "action" in rpt and rpt.action:
            ...
            if rpt.action == "fit":
                _add_cells_fit(nb, x_var, y_var, x_range_inds_var, rpt)
...
def _add_cells_cluster(
    notebook, data_var, fields, col_limit, x_var, y_var, x_label, y_label, rpt
):
    notebook.add_code_cell("from sirepo.analysis import ml")
    clusters_var = "clusters"
    scale_var = "scale"
    cols = [idx for idx, f in enumerate(fields) if f and idx < col_limit]
...
def _add_cells_fft(notebook, x_var, y_var, x_label):
    ...
    y_info = [PKDict(y_var=y_norm_var, y_label="", style="line")]
    notebook.add_report(
        PKDict(x_var=w_var, x_label=x_label, y_info=y_info, title="FFT")
    )
    ...
    for c in curves:
        y_info.append(
            PKDict(
                y_var=_var("y", c),
                x_points=x_fit_var,
                y_label=c.capitalize(),
                style="line",
            ),
        )
    notebook.add_report(PKDict(x_var=x_var, x_label="", y_info=y_info, title="Fit"))
...
def _bpm_readings_for_plots(data, history, start_time):
    ...
    t_indexes = numpy.where(
        (
            (all_times > current_time - time_window)
            if time_window > 0
            else (all_times >= 0)
        )
        & (all_times % period == 0)
    ...
    for t_idx in t_indexes[0]:
        ...
        x.append(xt)
        y.append(yt)
    return PKDict(x=x, y=y, z=z.tolist(), t=t)
        ...
        if t == "WATCH":
            ...
            for setting in ["hpos", "vpos"]:
                ...
                _MONITOR_TO_MODEL_FIELDS[mon_setting] = PKDict(
                    element=el.name, setting=setting
                )
        elif t == "KICKER":
            kicker_count += 1
            for setting in ["hkick", "vkick"]:
                mon_setting = "corrector{}_{}".format(
                    kicker_count, "HCurrent" if setting == "hkick" else "VCurrent"
                )
                _MONITOR_TO_MODEL_FIELDS[mon_setting] = PKDict(
                    element=el.name, setting=setting
                )
...
def _compute_clusters(report, plot_data, col_info):
    ...
    if report.clusterMethod == "kmeans":
        k_means = sklearn.cluster.KMeans(
            init="k-means++",
            n_clusters=count,
            n_init=report.clusterKmeansInit,
            random_state=report.clusterRandomSeed,
        )
        k_means.fit(x_scale)
        k_means_cluster_centers = numpy.sort(k_means.cluster_centers_, axis=0)
        k_means_labels = sklearn.metrics.pairwise.pairwise_distances_argmin(
            x_scale, k_means_cluster_centers
        )
        group = k_means_labels
    elif report.clusterMethod == "gmix":
        gmm = sklearn.mixture.GaussianMixture(
            n_components=count, random_state=report.clusterRandomSeed
        )
        gmm.fit(x_scale)
        group = gmm.predict(x_scale)
    elif report.clusterMethod == "dbscan":
        db = sklearn.cluster.DBSCAN(eps=report.clusterDbscanEps, min_samples=3).fit(
            x_scale
        )
        group = db.fit_predict(x_scale) + 1.0
        count = len(set(group))
    elif report.clusterMethod == "agglomerative":
        agg_clst = sklearn.cluster.AgglomerativeClustering(
...
def _data_cell(path, var_name):
    ...
    with pkio.open_text(path) as f:
        ...
        for r in reader:
            if not _is_data(r):
                continue
            d.append(f"    {[float(col) for col in r]},")
    # for legal JSON
    if len(d) > 1:
        re.sub(r",$", "", d[-1])
    d.append("])")
...
def _fit_to_equation(x, y, equation, var, params):
    ...
    eq_ops_rejected = [
        op for op in eq_ops if op not in SCHEMA.constants.allowedEquationOps
    ]
    assert len(eq_ops_rejected) == 0, sirepo.util.err(
        eq_ops_rejected, "operation fobidden"
    )
    assert _validate_eq_var(var), sirepo.util.err(var, "invalid variable name")
    assert all(
        [_validate_eq_var(p) for p in re.split(r"\s*,\s*", params)]
...
def _load_file_with_history(report, path, col_info):
    res = numpy.genfromtxt(
        path, delimiter=",", skip_header=col_info["header_row_count"]
    )
    if "history" in report:
        for action in report.history:
            if action.action == "trim":
                ...
                res = res[
                    (res[:, idx] * scale >= action.trimMin)
                    & (res[:, idx] * scale <= action.trimMax)
                ]
...
def _monitor_data_for_plots(data, history, start_time, type):
    ...
    for mon_setting in history:
        ...
        t_deltas = [
            round(((dt.days * 86400) + dt.seconds + (dt.microseconds / 1000000)))
            for dt in [t - start_time for t in h.times]
        ]
        pos = numpy.full(len(t_deltas), _position_of_element(data, el["_id"]))
        m_data[el_name][el_setting] = PKDict(
            vals=h.vals, times=t_deltas, position=pos.tolist()
        )
    return m_data
    ...
    for line in pkio.read_text(str(monitor_path)).split("\n"):
        ...
        if not history:
            ...
            if var_name not in monitor_values:
                monitor_values[var_name] = PKDict(vals=[], times=[])
...
def _setting_plots_by_position(data, history, start_time):
    ...
    for k_idx, k in enumerate(k_sorted):
        for s in k[1]:
            ...
            t_indexes = numpy.where(
                (
                    (times > current_time - time_window)
                    if time_window > 0
                    else (times >= 0)
                )
                & (times % period == 0)
...
def _setting_plots_by_time(data, history, start_time):
    ...
    for k_name in sorted([k for k in kickers]):
        ...
        for s in sorted([s for s in k_s]):
            ...
            k_sorted[-1][1].append(
                PKDict(setting=s, vals=k_s[s]["vals"], times=k_s[s]["times"]),
            )
