...
def generate_rtdose_file(data, run_dir):
    ...
    with h5py.File(dose_hd5, "r") as f:
        ...
        ds.GridFrameOffsetVector = np.linspace(
            0.0, frame["spacing"][2] * (shape[0] - 1), shape[0]
        ).tolist()
...
def sim_frame(frame_args):
    ...
    if frame_args.frameReport.startswith("dicomAnimation"):
        ...
        res = simulation_db.read_json(
            _dicom_path(model_data["models"]["simulation"], plane, frame_index)
        )
        res["pixel_array"] = _read_pixel_plane(plane, frame_index, model_data)
        return res
    if frame_args.frameReport == "dicomDose":
        return {"dose_array": _read_dose_frame(frame_index, model_data)}
...
def write_parameters(data, run_dir, is_parallel):
    ...
    if data["report"] in ("doseCalculation", "dvhReport"):
        _, roi_models = _generate_rtstruct_file(
            _parent_dir(run_dir), _parent_dir(run_dir)
        )
...
def _extract_series_frames(simulation, dicom_dir):
    ...
    if not selected_series:
        raise RuntimeError(
            "No series found with {} orientation".format(_EXPECTED_ORIENTATION)
        )
    if rt_struct_path:
        res["regionsOfInterest"] = _summarize_rt_structure(
            simulation, dicom.read_file(rt_struct_path), frames.keys()
        )
    sorted_frames = []
...
def _generate_rtstruct_file(sim_dir, target_dir):
    ...
    plan = _create_dicom_dataset(
        frame_data["StudyInstanceUID"], "RT_STRUCT", "RTSTRUCT"
    )
...
def _histogram_from_pixels(pixels):
    ...
    e = [
        ...
        np.floor(extent[1] / step) * step + step * 0.5,
        step,
    ]
    bins = int(np.ceil((e[1] - e[0]) / e[2]))
    hist, edges = np.histogram(
        pixels, bins=bins, range=[e[0], e[0] + (bins - 1) * step]
    )
    if hist[0] == hist.max():
        v = hist[0]
        hist[0] = 0
...
def _read_pixel_plane(plane, idx, data):
    ...
    size = [
        plane_info["c"]["frameCount"],
        plane_info["s"]["frameCount"],
        plane_info["t"]["frameCount"],
    ]
    frame = []
...
def _summarize_rt_structure(simulation, plan, frame_ids):
    ...
    simulation_db.write_json(
        _roi_file(simulation["simulationId"]),
        {
            "models": {
