...
def set_optics(_v):
    ...
    if len(_v.op_fin) > 0:
        if _v.op_fin not in arElNamesAll:
            raise Exception(
                'Optical element with the name specified in the "op_fin" option is not present in this beamline'
            )
    ...
    # S0 (primary slit)
    if "S0" in arElNames:
        el.append(SRWLOptA("r", "a", _v.op_S0_dx, _v.op_S0_dy, _v.op_S0_x, _v.op_S0_y))
        pp.append(_v.op_S0_pp)

    # Drift S0 -> HDM
    if "S0_HDM" in arElNames:
        el.append(SRWLOptD(zHDM - zS0))
        pp.append(_v.op_S0_HDM_pp)

    # Drift S0 -> S1
    if "S0_S1" in arElNames:
        el.append(SRWLOptD(zS1 - zS0))
        pp.append(_v.op_S0_S1_pp)
    ...
    if "HDM" in arElNames:
        ...
        if len(ifnHDM) > 0:
            ...
            opHDM = srwl_opt_setup_surf_height_1d(
                hProfDataHDM,
                "x",
                _ang=angHDM,
                _amp_coef=_v.op_HDM_amp,
                _nx=1000,
                _ny=200,
                _size_x=horApHDM,
                _size_y=verApHDM,
            ...
            ofnHDM = (
                os.path.join(_v.fdir, _v.op_HDM_ofn) if len(_v.op_HDM_ofn) > 0 else ""
            )
            if len(ofnHDM) > 0:
            ...
            el.append(opHDM)
            pp.append(_v.op_HDM_pp)

    # Drift HDM -> S1
    if "HDM_S1" in arElNames:
        el.append(SRWLOptD(zS1 - zHDM + _v.op_S1_dz))
        pp.append(_v.op_HDM_S1_pp)

    # S1 slit
    if "S1" in arElNames:
        el.append(SRWLOptA("r", "a", _v.op_S1_dx, _v.op_S1_dy, _v.op_S1_x, _v.op_S1_y))
        pp.append(_v.op_S1_pp)

    # Drift S1 -> DCM
    if "S1_DCM" in arElNames:
        el.append(SRWLOptD(zDCM - zS1))
        pp.append(_v.op_S1_DCM_pp)

    # Double-Crystal Monochromator
    tCr1 = [0, 0, -1]  # required for surface error
    if "DCM" in arElNames:
        tc = 1e-02  # [m] crystal thickness
        angAs = 0.0 * 3.1415926 / 180.0  # [rad] asymmetry angle
        hc = [1, 1, 1]
        ...
        orientDataCr2 = opCr2.find_orient(
            _en=_v.op_DCM_e, _ang_dif_pl=3.1415926
        ...
        el.append(opCr2)
        pp.append(_v.op_DCMC2_pp)

    # DCM Surface Error
    horApDCM = 2.0e-03  # Projected dimensions
    verApDCM = 2.0e-03
    ...
    if len(ifnDCME) > 0:
        ...
        opDCME = srwl_opt_setup_surf_height_1d(
            hProfDataDCME,
            "y",
            _ang=angDCM,
            _amp_coef=_v.op_DCME_amp,
            _nx=1000,
            _ny=200,
            _size_x=horApDCM,
            _size_y=verApDCM,
        ...
        ofnDCME = (
            os.path.join(_v.fdir, _v.op_DCME_ofn) if len(_v.op_DCME_ofn) > 0 else ""
        )
        if len(ofnDCME) > 0:
        ...
        el.append(opDCME)
        pp.append(_v.op_DCME_pp)

    # Drift DCM -> S2
    if "DCM_S2" in arElNames:
        el.append(SRWLOptD(zS2 - zDCM + _v.op_S2_dz))
        pp.append(_v.op_DCM_S2_pp)
    ...
    if "S1_S2" in arElNames:
        el.append(SRWLOptD(zS2 - zS1 + _v.op_S2_dz))
        pp.append(_v.op_S1_S2_pp)

    # S2 slit
    if "S2" in arElNames:
        el.append(SRWLOptA("r", "a", _v.op_S2_dx, _v.op_S2_dy, _v.op_S2_x, _v.op_S2_y))
        pp.append(_v.op_S2_pp)

    # Drift S2 -> BPM
    if "S2_BPM" in arElNames:
        el.append(SRWLOptD(zBPM - zS2 + _v.op_BPM_dz))
        pp.append(_v.op_S2_BPM_pp)

    # Drift BPM -> CRL
    if "BPM_CRL" in arElNames:
        el.append(SRWLOptD(zCRL - zBPM + _v.op_CRL_dz))
        pp.append(_v.op_BPM_CRL_pp)

    # Drift S2 -> CRL
    if "S2_CRL" in arElNames:
        el.append(SRWLOptD(zCRL - zS2 - _v.op_S2_dz + _v.op_CRL_dz))
        pp.append(_v.op_S2_CRL_pp)
    ...
    if "CRL_KL" in arElNames:
        el.append(SRWLOptD(zKL - zCRL - _v.op_CRL_dz + _v.op_KL_dz))
        pp.append(_v.op_CRL_KL_pp)

    # Drift CRL -> Sample
    if "CRL_SMP" in arElNames:
        el.append(SRWLOptD(zSample - zCRL - _v.op_CRL_dz + _v.op_SMP_dz))
        pp.append(_v.op_CRL_SMP_pp)

    # KL Aperture
    if "KLA" in arElNames:
        el.append(
            SRWLOptA("r", "a", _v.op_KLA_dx, _v.op_KLA_dy, _v.op_KL_x, _v.op_KL_y)
        )
        pp.append(_v.op_KLA_pp)

    # KL (1D, horizontally-focusing)
    if "KL" in arElNames:
        el.append(
            SRWLOptL(_v.op_KL_fx, _v.op_KL_fy, _v.op_KL_x, _v.op_KL_y)
        )  # KL as Ideal Lens; to make it a transmission element with a profile read from a file
        pp.append(_v.op_KL_pp)

    # Drift KL -> S3
    if "KL_S3" in arElNames:
        el.append(SRWLOptD(zS3 - zKL + _v.op_S3_dz))
        pp.append(_v.op_KL_S3_pp)

    # S3 slit
    if "S3" in arElNames:
        el.append(SRWLOptA("r", "a", _v.op_S3_dx, _v.op_S3_dy, _v.op_S3_x, _v.op_S3_y))
        pp.append(_v.op_S3_pp)

    # Drift S3 -> Sample
    if "S3_SMP" in arElNames:
        el.append(SRWLOptD(zSample - zS3 + _v.op_SMP_dz))
        pp.append(_v.op_S3_SMP_pp)
    ...
    if "SMP" in arElNames:
        ...
        if len(ifnSMP) > 0:
            ...
            ofnSMP = (
                os.path.join(_v.fdir, _v.op_SMP_ofn) if len(_v.op_SMP_ofn) > 0 else ""
            )
            if len(ofnSMP) > 0:
            ...
            el.append(opSMP)
            pp.append(_v.op_SMP_pp)
            ifSMP.close()

    # Drift Sample -> Detector
    if "SMP_D" in arElNames:
        el.append(SRWLOptD(zD - zSample + _v.op_D_dz))
        pp.append(_v.op_SMP_D_pp)
...
varParam = [
    ...
    [
        "fdir",
        "s",
        os.path.join(os.getcwd(), "data_CHX"),
    ...
    [
        ...
        "s",
        "FIB",
        "name of the final optical element wavefront has to be propagated through",
    ],
    ...
    ["op_S0_x", "f", 0.0, "slit S0: horizontal center position [m]"],
    ["op_S0_y", "f", 0.0, "slit S0: vertical center position [m]"],
    [
        "op_HDM_ifn",
        "s",
        "CHX_HDM_height_prof_1d.dat",
    ...
    [
        ...
        "f",
        1.0,
        "mirror HDM: amplification coefficient for height profile data",
    ],
    [
        "op_HDM_ofn",
        "s",
        "res_CHX_HDM_opt_path_dif.dat",
        "mirror HDM: output file name of optical path difference data",
    ],
    ["op_HDM_x", "f", 0.0, "mirror HDM surface error: horizontal center position [m]"],
    ["op_HDM_y", "f", 0.0, "mirror HDM surface error: vertical center position [m]"],
    ["op_S1_dz", "f", 0.0, "S1: offset of longitudinal position [m]"],
    ["op_S1_dx", "f", 0.2e-03, "slit S1: horizontal size [m]"],
    ["op_S1_dy", "f", 1.0e-03, "slit S1: vertical size [m]"],
    ["op_S1_x", "f", 0.0, "slit S1: horizontal center position [m]"],
    ["op_S1_y", "f", 0.0, "slit S1: vertical center position [m]"],
    ["op_DCM_e", "f", 9000.0, "DCM: central photon energy DCM is tuned to [eV]"],
    [
        "op_DCM_ac1",
        "f",
        0.0,
        "DCM: angular deviation of 1st crystal from exact Bragg angle [rad]",
    ],
    [
        "op_DCM_ac2",
        "f",
        0.0,
        "DCM: angular deviation of 2nd crystal from exact Bragg angle [rad]",
    ],
    [
        "op_DCME_ifn",
        "s",
        "CHX_DCM_height_prof_1d.dat",
        "DCM surface error: input file name of height profile data",
    ],
    ["op_DCME_amp", "f", 1.0, "DCM surface error: amplification coefficient"],
    [
        "op_DCME_ofn",
        "s",
        "res_CHX_DCM_opt_path_dif.dat",
        "DCM surface error: output file name of optical path difference data",
    ],
    ["op_DCME_x", "f", 0.0, "DCM surface error: horizontal center position [m]"],
    ["op_DCME_y", "f", 0.0, "DCM surface error: vertical center position [m]"],
    ["op_FIB_fpl", "s", "", 'FIB: focusing plane ("h" or "v" or "hv" or "")'],
    [
        "op_FIB_delta_e",
        "f",
        4.20756805e-06,
        "Fiber: refractive index decrement of main (exterior) material",
    ],
    [
        "op_FIB_delta_c",
        "f",
    ...
    [
        ...
        "f",
        7312.94e-06,
        "Fiber: attenuation length of main (exterior) material [m]",
    ],
    [
        "op_FIB_atnl_c",
        "f",
    ...
    ["op_FIB_d_e", "f", 100.0e-06, "Fiber: ext. diameter [m]"],
    ["op_FIB_d_c", "f", 10.0e-06, "Fiber: core diameter [m]"],
    ["op_FIB_x", "f", 0.0, "Fiber: horizontal center position [m]"],
    ["op_FIB_y", "f", 0.0, "Fiber: vertical center position [m]"],
    ["op_S2_dz", "f", 0.0, "S2: offset of longitudinal position [m]"],
    ["op_S2_dx", "f", 0.05e-03, "slit S2: horizontal size [m]"],
    [
        "op_S2_dy",
        "f",
        0.2e-03,
        "slit S2: vertical size [m]",
    ],  # 1.0e-03, 'slit S2: vertical size [m]'],
    ["op_S2_x", "f", 0.0, "slit S2: horizontal center position [m]"],
    ["op_S2_y", "f", 0.0, "slit S2: vertical center position [m]"],
    ["op_BPM_dz", "f", 0.0, "BPM: offset of longitudinal position [m]"],
    ["op_CRL_dz", "f", 0.0, "CRL: offset of longitudinal position [m]"],
    ...
    [
        ...
        "f",
        1.0e-03,
        "CRL1: geometrical aparture of 1D CRL in the plane where there is no focusing",
    ],
    [
        "op_CRL1_apf",
        "f",
        2.4e-03,
        "CRL1: geometrical aparture of 1D CRL in the focusing plane",
    ],
    [
        "op_CRL1_rmin",
        "f",
        1.5e-03,
        "CRL1: radius of curface curvature at the tip of parabola [m]",
    ],
    ...
    ["op_CRL1_x", "f", 0.0, "CRL1: horizontal center position [m]"],
    ["op_CRL1_y", "f", 0.0, "CRL1: vertical center position [m]"],
    ...
    [
        ...
        "f",
        1.0e-03,
        "CRL2: geometrical aparture of 1D CRL in the plane where there is no focusing",
    ],
    [
        "op_CRL2_apf",
        "f",
        1.4e-03,
        "CRL2: geometrical aparture of 1D CRL in the focusing plane",
    ],
    [
        "op_CRL2_rmin",
        "f",
        0.5e-03,
        "CRL2: radius of curface curvature at the tip of parabola [m]",
    ],
    ...
    ["op_CRL2_x", "f", 0.0, "CRL2: horizontal center position [m]"],
    ["op_CRL2_y", "f", 0.0, "CRL2: vertical center position [m]"],
    [
        "op_KLA_dx",
        "f",
        1.0e-03,
        "KL aperture: horizontal size [m]",
    ],  # 1.4e-03, 'KL Aperture: horizontal size [m]'],
    [
        "op_KLA_dy",
        "f",
        0.1e-03,
        "KL aperture: vertical size [m]",
    ],  # 0.2e-03, 'KL Aperture: vertical size [m]'],
    ["op_KL_dz", "f", 0.0, "KL: offset of longitudinal position [m]"],
    ["op_KL_fx", "f", 3.24479, "KL: horizontal focal length [m]"],
    ["op_KL_fy", "f", 1.0e23, "KL: vertical focal length [m]"],
    ["op_KL_x", "f", 0.0, "KL: horizontal center position [m]"],
    ["op_KL_y", "f", 0.0, "KL: vertical center position [m]"],
    ["op_S3_dz", "f", 0.0, "S3: offset of longitudinal position [m]"],
    ["op_S3_dx", "f", 10.0e-06, "slit S3: horizontal size [m]"],
    ["op_S3_dy", "f", 10.0e-06, "slit S3: vertical size [m]"],
    ["op_S3_x", "f", 0.0, "slit S3: horizontal center position [m]"],
    ["op_S3_y", "f", 0.0, "slit S3: vertical center position [m]"],
    ["op_SMP_dz", "f", 0.0, "sample: offset of longitudinal position [m]"],
    [
        "op_SMP_ifn",
        "s",
        "CHX_SMP_CDI_001.pickle",
        'sample: model file name (binary "dumped" SRW transmission object)',
    ],
    [
        "op_SMP_ofn",
        "s",
        "res_CHX_SMP_opt_path_dif.dat",
        "sample: output file name of optical path difference data",
    ],
    ["op_SMP_x", "f", 0.0, "sample: horizontal center position [m]"],
    ["op_SMP_y", "f", 0.0, "sample: vertical center position [m]"],
    ["op_D_dz", "f", 0.0, "detector: offset of longitudinal position [m]"],
    ...
    [
        "op_S0_pp",
        "f",
        [0, 0, 1, 0, 0, 2.5, 5.0, 1.5, 2.5, 0, 0, 0],
        "slit S0: propagation parameters",
    ],
    [
        "op_S0_HDM_pp",
        "f",
    ...
    [
        ...
        "f",
        [0, 0, 1, 0, 0, 1.0, 1.0, 1.0, 1.0, 0, 0, 0],
        "slit S1: propagation parameters",
    ],
    [
        "op_S1_DCM_pp",
        "f",
    ...
    [
        ...
        "f",
        [0, 0, 1, 0, 0, 1.0, 1.0, 1.0, 1.0, 0, 0, 0],
        "slit S2: propagation parameters",
    ],
    [
        "op_S2_BPM_pp",
        "f",
    ...
    [
        ...
        "f",
        [0, 0, 1, 0, 0, 1.0, 1.0, 1.0, 1.0, 0, 0, 0],
        "slit S3: propagation parameters",
    ],
    ...
    [
        "op_SMP_D_pp",
        "f",
        [0, 0, 1, 3, 0, 1.0, 1.0, 1.0, 1.0, 0, 0, 0],
